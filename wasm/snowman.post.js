...
(function() {
     ...
     * @return {Promise}
     */
    function download(url, path, force) {
        return async(function*() {
            let stat = FS.analyzePath(path);
            if(stat.exists && !force) {
                console.debug(`Skipping download of ${url} cause the target already exists!`);
                return;
            }
